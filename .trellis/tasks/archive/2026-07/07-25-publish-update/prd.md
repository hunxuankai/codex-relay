...
- [x] npm、主 Tauri crate、`codex-relay-core` crate、锁文件与发布结构测试一致为 `0.2.0`。
- [x] 最终中文发布说明准确覆盖本轮主要新增能力、升级方式、未知发布者与数据保留边界。
- [x] 本轮 `npm run check` 退出码为 0，测试数量和真实失败均已记录。
- [x] 无 updater 私钥的普通 `npm run build` 退出码为 0，并记录实际 EXE/NSIS 产物元数据与 SHA-256。
- [x] 候选提交已推送到远端 `main`，GitHub Actions 发布工作流成功生成经核对的 Draft。
- [x] Draft 的版本、目标提交、说明、NSIS、`.sig`、`latest.json`、平台 URL 与签名关系全部一致。
- [x] `v0.2.0` 已公开，公开端点与资产复核通过且未发生漂移。
- [x] 已在隔离环境执行 `v0.1.2 → v0.2.0` 应用内升级；若环境或人工交互阻止完成，报告中明确保留未验证项，不虚报成功。
- [x] Git、日志、任务材料、测试输出与发布说明不含真实密钥、完整认证文件、Authorization Header 或真实用户数据。
