...
- [ ] AC3：候选提交已精确推送到 GitHub `main`，远端提交与发布工作流输入一致。
- [ ] AC4：`v0.3.0` 基线安装器已安全暂存并核对；发布工作流成功生成经逐项审计的
      `v0.4.0` Draft，资产与清单满足 updater 契约且不含秘密或用户数据。
- [ ] AC5：`v0.4.0` 已公开为 Latest，公开端点与资产复核一致，历史 Release/tag 清理
      Run 成功且只保留当前 Latest。
- [ ] AC6：实际执行的 Sandbox/VM 准备与升级结果、真实失败和所有未执行项均以本轮
