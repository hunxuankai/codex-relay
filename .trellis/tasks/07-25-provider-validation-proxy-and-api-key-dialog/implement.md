...
5. [部分完成] 为 Rust command 和 core service 写失败测试：代理未启用的稳定错误、无设置文件写入和 command 请求 ID 校验已完成；全量 core 与路径安全测试已通过。当前 Tauri Rust watcher 运行时，标准 provider-workflow npm 入口按门禁未运行。
6. [已完成] 更新 command、service、API HTTP/Codex 网关调用链；不添加任何配置写入或秘密日志。
7. [部分完成] 运行专项测试、类型检查、Rust 格式/库测试和任务级质量检查；审查 diff、密钥命中、路径隔离与规范更新需求。标准 `npm run check` 与提交前 workspace Rust 门禁等待当前 watcher 停止或切换为 no-watch。
