...
- [ ] 修改 Codex、Tauri 或其他第三方配置前，先查当前官方 schema/文档并记录字段所有权。
- [ ] 区分“外部工具当前生效配置”和“Relay 自己的 UI 偏好”；后者放在独立版本化应用数据中。
- [ ] 验证 UI → command → service → 文件的写入流，确认私有元数据不会污染外部配置块。
- [ ] 为每个新增受管文件同步扩展路径、指纹、事务、备份、恢复、监控、自检、DTO 和打开白名单。
...
当一个仓库同时生成主应用前端、便携维护工具、安装包或其他本地产物时：

- [ ] 哪个构建拥有并可整体清理每个输出根？
- [ ] 是否有运行中的 EXE、日志或维护产物被放进另一个构建会递归清理的目录？
- [ ] Windows 文件锁下，保持工具运行时执行拥有者构建是否仍能成功？
- [ ] 默认路径、忽略规则、文档、包装脚本和结构测试是否来自同一交付契约？
- [ ] 是否通过独立产物根解决所有权，而不是关闭清理并保留可能过期的哈希资产？

发布控制台的具体路径与真实回归要求见 `../release/publishing.md` 的
“发布控制台交付目录所有权契约”。
