...
- 修改 Provider 的 Base URL、API Key、模型偏好或当前选择；
...
| `provider-preferences.json` v2 | 命名 Base URL 列表与可选模型偏好 | 不保存 API Key；不覆盖未知 Provider 记录 |
...
数组顺序是展示顺序。重命名、替换值和切换不改变顺序；新增项只追加到末尾。
...
- Rust 纯逻辑：名称/值唯一、稳定 ID、顺序、选中项、最后项、v1/v2、未知版本与脱敏 `Debug`。
...
- composable： mutation 后权威刷新、busy 防重、Provider 事件晚响应、密钥 manager 的
  load/save/clear/scope dispose/晚响应丢弃。
- Vue：创建四个初始字段、编辑模式无 URL/Key 修改、两行 `ElSegmented` 独立事件、
