...
- AI 根据请求的规模、复杂度、风险和持久化需要自主决定是否创建 Trellis 任务。
- 一次性答复、简单只读查询，以及范围清楚、低风险、局部且可在当前会话内完成并
  验证的小改动，可以不创建任务。
- 跨模块或多阶段、需求或方案复杂、涉及安全/配置迁移/发布/卸载/数据保留、
  需要持久化 PRD/设计/进度/验证证据、可能跨会话，或用户明确要求使用 Trellis
  时，必须创建任务。
- 判断需要任务时直接创建并简短告知用户，无需另行征得任务创建许可。已有活动
  任务时先确认范围相关；相关则继续使用，不相关则不得把新工作混入旧任务。
- 自动创建任务不等于获得实施授权。仍然必须先完成规划，并确认用户已明确要求
  或批准实施。
...
没有活动任务。AI 根据请求的规模、复杂度、风险和持久化需要自主决定是否创建 Trellis 任务。
一次性答复、简单只读查询，以及范围清楚、低风险、局部且可在当前会话内完成并验证的小改动，可以直接处理。
跨模块或多阶段、需求或方案复杂、涉及安全/配置迁移/发布/卸载/数据保留、需要持久化规划或证据、可能跨会话，或用户明确要求使用 Trellis 时，直接创建任务并简短告知用户，无需另行征得创建许可。
[/workflow-state:no_task]

### Phase 1：规划
- 1.0 创建任务 `[required · once]`（仅在判断需要 Trellis 任务时）
...
路由前先确认当前请求属于活动任务；若不相关，保持旧任务原状，并按“请求分类”
决定直接处理或创建新任务。
...
- 自动创建任务不等于获得实施授权；确认用户已明确要求或批准实施，并完成材料
  审查后运行 `task.py start`，才能开始实施。
...
目标：对请求分类；判断需要 Trellis 任务时直接创建并告知用户，再生成实施前所需
的规划材料。

#### 1.0 创建任务 `[required · once]`

当分类结果需要 Trellis 任务时，直接创建任务目录并简短告知用户，不另行征求任务
创建许可。该命令将 status 设为 `planning`、写入 `task.json`、创建默认 `prd.md`，
并在会话身份可用时自动将新任务设为目标：
...
- 没有活动任务时必须按规模、复杂度、风险和持久化需要分类；需要任务时直接创建
  并告知用户，不逐次征求任务创建许可。
