...
Release 使用本地 CSP、per-machine NSIS 安装与仓库图标。安装器需要管理员权限；首次安装仅在 `D:` 为固定磁盘时优先使用 `D:\Program Files\Codex Relay`，否则回退到系统 Program Files，后续 per-machine 升级沿用已登记目录。旧 current-user 安装不会自动迁移，发布时应要求用户先卸载旧程序；卸载仍保留 Codex 配置和应用数据。仓库不含签名证书。未签名构建可能显示“未知发布者”，不能把构建成功描述为已签名。
