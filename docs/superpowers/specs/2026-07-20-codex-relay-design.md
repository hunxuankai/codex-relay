...
Tauri 配置的产品名为 `Codex Relay`，主程序名为 `CodexRelay.exe`，Windows bundle 使用 NSIS、per-machine 安装和开始菜单入口。首次安装在固定磁盘 `D:` 存在时优先使用 `D:\Program Files\Codex Relay`，否则使用系统 Program Files；后续 per-machine 升级沿用原安装目录。旧 current-user 安装不自动迁移，用户应先卸载旧程序。卸载不主动删除 Codex 配置、应用备份或其他用户数据。无代码签名证书时不声明已签名，并在 README 说明 Windows 可能显示“未知发布者”。
