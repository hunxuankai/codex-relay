...
- `AGENTS.md`：不可违反的全仓库开发、安全和文档语言规则；
...
1. 删除根目录 `项目提示词.txt`；
2. 修改 `AGENTS.md`，要求项目文档、设计文档、实施计划、验证报告和 README 默认使用
   简体中文，命令、代码标识符、文件名和必要技术专有名词可以保留原文；
3. 修改 `docs/superpowers/specs/2026-07-20-codex-relay-design.md`，将旧提示词引用改为
   已批准设计和项目验收要求；
4. 修改 `docs/superpowers/plans/2026-07-20-codex-relay-implementation.md`，从历史暂存命令中
   移除已删除文件；
5. 修改 `docs/verification-report.md`，将旧提示词引用改为正式发布要求；
6. 不修改应用源码、依赖、构建配置或运行行为。
...
验收结果：文件不存在；`AGENTS.md` 包含简体中文文档规则；三个被清理的旧文档不再引用
该文件；本决策记录和对应实施计划可以为了保留审计依据而提及被删除文件；Git diff 没有
空白错误；改动范围仅为仓库规则、设计文档、历史文档引用和文件删除。由于不改变可执行
代码，不要求重新运行前后端测试或 Release 构建。
