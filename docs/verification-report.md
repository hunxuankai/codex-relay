...
- Debug、Release 和 NSIS per-machine 安装包构建配置；首次安装优先固定磁盘 `D:`，否则
  回退系统 Program Files，升级沿用原目录。
...
2026-07-20T11:38:22.7402129+08:00 开始执行最新 `npm run check`，前端 Vitest 通过
15/15 个测试文件和 64/64 个测试，退出码为 0。新增测试锁定 per-machine 模式、自定义 NSIS
模板、固定 D 盘优先、Program Files 回退、升级目录优先级和目录选择页。

### 10. Rust 测试结果

同一次最新完整检查通过 107 个 Rust 单元测试、2 个路径安全集成测试和 1 个 Provider
工作流集成测试，失败数为 0。

### 11. TypeScript 检查结果

`vue-tsc --noEmit -p tsconfig.app.json` 作为 2026-07-20 12:05 左右开始的本轮最终
`npm run check` 第一阶段执行，退出码为 0。

### 12. Clippy 结果

`cargo fmt --check --manifest-path src-tauri/Cargo.toml` 与
`cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets --all-features -- -D warnings`
均在本轮最终 `npm run check` 中退出 0，完整命令耗时 44.5 秒，Clippy 没有 warning。
...
执行时间：2026-07-20 12:06 左右开始，完整命令耗时 219.9 秒
...
最后写入：2026-07-20T12:10:05+08:00

SHA-256：`D9FAA69450C6C4BE24A37A8D6DEF53E6590F5FDDC61A1A313B7F9571A86BFA91`
...
SHA-256：`5ED860265E4EE57944097710CCA929FF74B170DE9474BF04D9106533B8A5BBD8`

安装器模式：`perMachine`，需要管理员权限。提取安装器 manifest 后确认
`requestedExecutionLevel="requireAdministrator"`。

首次安装目录逻辑：当前机器 `D:` 的 Windows DriveType 为 3（固定磁盘），因此默认建议
`D:\Program Files\Codex Relay`；没有固定 D 盘时回退 `$PROGRAMFILES64\Codex Relay`。
`RestorePreviousInstallLocation` 在默认目录选择之后执行，后续 per-machine 升级仍优先使用
已登记目录；旧 current-user 安装记录位于 HKCU，不会自动迁移，用户需先卸载旧程序。
标准 `MUI_PAGE_DIRECTORY` 保留，用户可以修改建议路径。
...
- 条件安装目录改动后的 `npm run check`：2026-07-20T11:38:22.7402129+08:00 至
  2026-07-20T11:41:34.5578764+08:00，退出码 0；前端 64/64，Rust 107+2+1。
- 第一次重新打包于 2026-07-20T11:41:51.0270456+08:00 开始，Rust Release 编译成功，
  但旧 `CodexRelay.exe` 进程短暂锁住目标文件，重命名时报 Windows 错误 5，退出码 1；该次
  没有进入 NSIS 模板编译，不计为打包成功。锁定进程随后自行退出，两个 Release 文件均
  通过独占打开检查。
- 解锁后的 `npm run build` 重试：2026-07-20T11:47:47.6670965+08:00 至
  2026-07-20T11:50:55.2755254+08:00，退出码 0；`makensis` 成功编译自定义 Tauri 2.11.4
  模板并生成新的 per-machine NSIS 安装器。
- 提交前再次运行完整 `npm run check`，耗时 44.5 秒，退出码 0；前端 64/64，Rust
  107+2+1，TypeScript、格式检查和全目标全特性 Clippy 均通过。
- 提交前再次运行 `npm run build`，耗时 219.9 秒，退出码 0；最终 Release 与 NSIS
  产物在 2026-07-20T12:10:05+08:00 写入，第 14 至 15 项已更新为本次文件信息。
...
- 条件 NSIS 安装目录变更的独立复审未发现 Critical、Important 或 Minor。复审核对了
  per-machine 注册上下文、固定 D 盘分支、Program Files 回退、目录选择页、后续升级目录
  恢复和卸载边界；实际安装、per-machine 升级及卸载仍未执行，并继续列为已知限制。
